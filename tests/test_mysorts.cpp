...
#define AMOUNT_TEST(testCode, counter) \
    {                                  \
        counter = testCode;            \
    }
...
{
    void (*generate)(int *, long long unsigned);
    char name[255];
};

void generateRandomArray(int *a, long long unsigned n)
{
    srand(time(NULL));
    for (int i = 0; i < n; ++i)
        a[i] = rand() % 10000;
...
void generateOrderedArray(int *a, long long unsigned n)
{
    for (int i = 0; i < n; ++i)
        a[i] = i;
}

void generateOrderedBackwards(int *a, long long unsigned n)
{
    for (int i = 0; i < n; ++i)
        a[i] = (int)n - i - 1;
}
...
void checkAmountCompares(long long (*sortFunc)(int *, int),
                         void (*generateFunc)(int *, long long unsigned),
                         long long unsigned size, char *experimentName)
{
    static long long unsigned runCounter = 1;
    static int innerBuffer[110000];
    generateFunc(innerBuffer, size);
    printf("Run #%zu| ", runCounter++);
    printf("Name: %s\n", experimentName);
    long long unsigned countOperations;
...
void amountComparesExperiment()
{
    SortWithInjectionFuncTag sorts[] = {
        ...
        {quickSort, "quickSort"},
        {heapSort, "heapSort"}};
    const long long unsigned FUNCS_N = ARRAY_SIZE(sorts);
    ...
    const long long unsigned CASES_N = ARRAY_SIZE(GeneratingFuncTags);
    for (long long unsigned size = 5; size <= 45; size += 5)
    {
        printf("------------------------------\n");
        printf("Size: %zu\n", size);
